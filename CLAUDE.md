...
2. **动作链三段式**：事件必须拍全"铺垫 → 瞬间 → 后果"（如：骑行→摔车→捡餐盒），
   不许跳过关键动作帧拿旁白糊弄。
3. **道具真实**：手机聊天/App 界面/票据类道具用 PIL 像素级合成（状态栏/头像/气泡/输入栏，
   头像从定妆图裁脸），不要 AI 直出的"示意图感"气泡卡。
4. **QC 环强制**：对白片段生成后必须 `transcribe` 回验台词逐字正确；每镜抽帧拼墙
   人工过目；不合格单镜重生成，不许带病装配。
5. **响度**：assemble 的 loudnorm 默认开（-14 LUFS），别关。
6. **时长预算**：先 TTS 后排轴——旁白实际时长必须 ≤ 所在镜头时长，超了改词或换镜头长度。
